...
def disburse_funds_from_bank_worker(disbursement_batch_control_id: str):
    ...
    with session_maker() as session:
        ...
        if not disbursement_envelope:
            _logger.error(f"No Disbursement Envelope found")
            return
        ...
        if not envelope_batch_status_for_cash:
            _logger.error(
                f"No EnvelopeBatchStatusForDigitalCash found"
            )
            return
...
def construct_disbursement_payloads_for_digital_cash(disbursement_batch_control_id, session, envelope, envelope_batch_status_for_digital_cash, sponsor_bank_configuration) -> List[DisbursementPaymentPayload]:
...
def construct_disbursement_payloads_for_physical_cash(disbursement_batch_control_id, session, envelope, envelope_batch_status_for_digital_cash, sponsor_bank_configuration) ->  List[DisbursementPaymentPayload]:
